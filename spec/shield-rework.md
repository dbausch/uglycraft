...
## Done when:
1. ✗ Shop state removed from all code paths; `LIFE_COST_PTS` gone from constants.py.
2. ✗ Pressing Enter in-game instantly buys shield (250 pts) with no screen change.
3. ✗ Shield auto-expires after 10 s; HUD shows live countdown in whole seconds.
4. ✗ On any collision the hitting enemy is teleported to BFS distance ≥ 8 (fallback ≥ 4).
5. ✗ Shielded hit: shield consumed, enemy respawned, no life lost.
6. ✗ Unshielded hit: life lost, player to start, hitting enemy still respawned.
