...
**One text colour throughout (spec 0072):** every HUD text element is `HUD_TEXT`; inactive/empty counters (`SHIELD --`, `BLOCKS 0`, `BRIDGES 0`) use `HUD_DIM = (115,92,48)`, a darker shade of the *same* hue, so active vs inactive still reads at a glance without a second colour. The seven old colours (`HUD_LIFE`/`GOLD`/`MAGENTA`/`RED`/`LTBLUE`/`LTGREEN`/`YELLOW`) are gone from the HUD; partial BLOCKS/BRIDGES state is conveyed by the `.` dot alone. Key **icons** keep their own colours (they are icons, not text).
...
5. **Key tracker** (spec 0071, extended by spec 0075 `_key_strip_element` → `KeyStackStrip`): one slot (`_KEY_SLOT=23`) per key colour **present in the current level** — `World._level_key_colours`, ordered by `KEY_NAMES`, exposed via `_WORLD_ATTRS` delegation. Since spec 0075 a colour may have **1–4** keys/doors (not unique), so each slot draws a **stack** of `World._level_key_counts[colour]` key icons overlaid down-right at `index*2` px: the loop draws a **ghost** key while `total − held > index` (the back keys) and a **lit** key otherwise (the front `held` ones, drawn last = on top at bottom-right). Icons are pre-rendered rimmed sprites `key_lit_{colour}` / `key_ghost_{colour}` (1 px dark rim `(12,10,8)` so overlaps stay countable; the ghost is the key colour blended 15 % over `HUD_BG`, **opaque** — full alpha — so it occludes cleanly). Totals are fixed for the level, so the strip width is constant (≈ old `n×23`, the 4-stack overhang is a few px) and the HUD never reflows; only `held` changes as keys are collected/spent (a used key reverts a front icon from lit to ghost — 2-state, current-inventory semantics). **A level with no keys omits the strip entirely** (returns `None` → not added to the HBox), space redistributed.
...
8. **BRIDGES counter** (spec 0072 D2 / 0073 D2): shown **only when the level contains planks** (`World._level_has_planks`, computed at level load), immediately left of BLOCKS; omitted (space redistributed) on plankless levels. Value = `_bridge_credits` (2 planks collected = 1, spec 0073 D2). Trailing indicator: `_` when no half is banked, or a **drawn lower-half block** when `_bridge_halves > 0` (a banked half). `HUD_TEXT` when ≥ 1 credit, else `HUD_DIM`.
9. **BLOCKS counter** (spec 0073 D2): value = `_block_credits` (a mined wall or a rubble each bank half a block; `HALVES_PER_CREDIT == 2`). Trailing indicator: `_` when no half is banked, or a **drawn lower-half block** when `_block_halves > 0`. `HUD_TEXT` if credits > 0, else `HUD_DIM`.
...
Each element is vertically centred by its own height (`cy = top + (STATUS_H - img.height)//2`), so the key strip and the shorter text share a common centre line. Keys are **not** unique per colour (spec 0075 / R-K2): the colour pool cycles, so a colour can have up to 4 keys/doors; the HUD conveys the count via the stacked-icon slot (above) rather than a numeric `×N`.
