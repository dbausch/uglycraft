...
9. **WALLS counter**: value = `_block_credits`. Trailing indicator: `_` when the crushed-wall count is even (no half credit), or a **drawn lower-half block** when `_block_halves > 0` (half an earned credit; `HALVES_PER_CREDIT == 2` so this is binary). `HUD_TEXT` if credits > 0, else `HUD_DIM`.
