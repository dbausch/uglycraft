...
**Dash leaders** (`hud.dash_fill`, spec 0072): every HUD string is tidied before rendering — any run of **> 2** spaces collapses to a `" -- "` leader (so a right-justified value stays linked to its label, e.g. `SCORE -- 0`), and a remaining trailing space becomes a `-` (so left-justified padding like `SEEK: Coin␣␣␣` reads `SEEK: Coin ---`, preserving the reserved width). Runs of ≤ 2 spaces (e.g. `LEVEL  1`) are left alone.
