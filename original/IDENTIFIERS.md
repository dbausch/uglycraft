...
| `Name` | High-score filename constant (`UGLI.HSC`) | `HighScoreFileName` |
...
| Identifier | Description | English proposal |
|---|---|---|
| `Steine` | Block-placement budget (*Steine* = stones) | `BlocksRemaining` |
...
| `Pausen` | Remaining pause tokens (*Pausen* = pauses) | `PausesRemaining` |
...
| `SX`, `SY` | Position of last-placed stone; redrawn after player moves (*Stein* = stone) | `BlockX`, `BlockY` |
| `LocX`, `LocY` | Current treasure X/Y location | `ItemX`, `ItemY` |
...
| `Sper` | Collision map; `true` = cell is blocked (*Sperrung* = blockage) | `Blocked` |
...
| `S` | Scratch string buffer (used with `Str()`, HUD writes, etc.) | *(keep)* |
...
| `ReStone` | Redraw all wall tiles from the collision map (*Re-Stein* = re-stone) | `DrawInner` |
| `Abfrage` | High-score name-entry and display sequence (*Abfrage* = inquiry) | `HighScoreEntry` |
| `Cls` | Thin wrapper around `ClrScr` | *(inline)* |
| `Verwirrung` | Animated splash/intro screen (*Verwirrung* = confusion) | `Intro` |
...
| `Wertsachen` | Display treasure list + game instructions (*Wertsachen* = valuables) | `ShowItemDescriptions` |
...
| `Rahmen` | Clear screen, draw border frame, initialise level (*Rahmen* = frame) | `DrawFrame` |
...
| `ZahlenSetzung` | Draw current treasure symbol at its location (*Zahlen-Setzung* = number placement) | `DrawItem` |
...
| `Laenge` | Display-column width of the string to centre (*Länge* = length) | `Cols` |
